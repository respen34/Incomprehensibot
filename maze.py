...
class Container:
    ...
    def add_inv(self, item):
        ...
        itemReference = self.search_inv(item.type)
        if itemReference is not None:
    ...
    def loot(self, itemType=None, amount=None):
        if itemType is not None:
            item = self.search_inv(itemType)
            ...
            if item is not None:
                ...
                elif amount < item.amount and amount > 0:
                    item.amount -= amount
...
class Item:
    ...
    def __init__(self, itemType, amount, special=None):
        self.type = itemType
        self.amount = amount
    ...
    def use(self, player, direction, target):
        ...
        if self.type in ('map'):
            ...
            if 'gas' in tile.tags and 'light' in message:
                ...
                inform = 'The room is ingulfed in a fireball.\n'
            ...
            if match:
                ...
                if direction:#blow up wall
                    directionIndex = ('north', 'east', 'south', 'west').index(direction)
                    tileList = [tile]
                    referenceTile = player.maze.maze.get(tile.reference[directionIndex])
                    if referenceTile:
                        tileList.append(referenceTile)

                    for tile in tileList:
                        optionList = [d for d in player.maze.tileTypes[tile.type]]
                        if optionList[directionIndex] == False:
                            optionList[directionIndex] = True
                        tile.type = player.maze.tileTypes.index(tuple(optionList))
...
class Player(Container):
    ...
    def move(self, direction):
        ...
        if self.maze.tileTypes[tile.type][direction]:
            referenceCoords = tile.reference[direction]

            if referenceCoords in self.maze.maze:
                newTile = self.maze.maze[referenceCoords]

                if self.maze.tileTypes[newTile.type][(direction + 2) % 4]: #move success, old tile

                    self.coords = referenceCoords
                    newTile.players.append(tile.players.pop(tile.players.index(self)))

                    message.append(newTile.description(self))

                else: #move failed, path blocked
                    #update type
                    moveOptions = [d for d in self.maze.tileTypes[tile.type]]
                    moveOptions[direction] = False
                    tile.type = self.maze.tileTypes.index(tuple(moveOptions))
                ...
                newTile = self.maze.maze[self.coords]
...
class Enemy(Container):#new addition, needs testing, needs ai, possible genetic learning during play.
    def __init__(self, Type, tile):
        self.type = Type
        self.tile = tile
        statDict = {'mimic':[50, 4, 2, 4, 0, self.bite]}
        default = [100, 3, 3, 3, 0, self.bite] #hp, str, dex, con, lck, *abilities
        stats = statDict.get(self.type, default)
...
class Tile(Container):
    ...
    def __init__(self, tileType, coords, referenceList, number, is_exit=False, probabilities=[]):
        '''tileType: an integer representing the shape of the tile
        coords: a 2-tuple representing the global postion on the maze dict
        referenceList(legacy): a list of references to adjacent tile
        probabilities(unused): list of probabilities of special types'''

        self.type = tileType
        self.coords = coords
        self.reference = referenceList #list of coords of adjacent tiles
        self.number = number
        ...
        if self.type <= 3 and r.random() <= .95:
            ...
            self.lootables['chest'] = self.chest #possible option to prevent looting of non-lootable special conditions
        elif self.type <= 3:
            self.chest = Chest()
            self.tags.append('chest')
    ...
    def move_options(self):
        ...
        options = [directionList[n] for n in range(4) if tile[n]]
        return(options)
    ...
    def description(self, player):
        ...
        if self.messages:
            messages.append("There's writting on the wall.")
...
class Merchant(Container):
    def __init__(self):
        self.inv = [Item('gold', 500),
                    ...
                    Item('rope', 10)]
        self.name = 'merchant'
        self.mood = r.random() #value between -1 and 1 represents agreeableness 1 being agreeable -1 being agressive
        #will affect the bartering system: .5 is the default value

        self.cost = {'gold':1, 'map':30-round(5*self.mood), 'match':2-round(self.mood), 'bomb':15-round(3 * self.mood),
    ...
    def buy(self, player, item_type, amount = None , offer = None):
        ...
        if item:
            if offer is None:
                if int(amount) <= item.amount and int(amount) > 0:
                    #buy at listed price
                    print('unfinished')
...
class maze:
    ...
    def old_gen_next(self, direction, oldCoords):
        '''generates the unexplored tile that a player moves to
           direction: the direction the player moved {0,3} [up, right, down, left]
           coords: the coordinates of the tile the player is moving from
           returns the index of the new tile
        '''
        tile = self.maze[oldCoords]
        coords = tile.reference[direction]
        
        referenceList = [(coords[0] + n, coords[1] + m) #list of coords of adjacent tiles
                        for n, m in ((0,1),(1,0),(0,-1),(-1,0))]
        direction = referenceList.index(oldCoords)
        tileOptions = [n for n in range(len(self.tileTypes))
                       if self.tileTypes[n][direction]] #dead, straight, elbow, elbow, T,T,T,4


        moveOptions = [d for d in self.tileTypes[tileType]]

        for c in referenceList:
            if c in self.maze:
                ref = (referenceList.index(c) + 2) % 4
                if self.tileTypes[self.maze[c].type][ref] == False:
                    moveOptions[referenceList.index(c)] = False
                    #print(f'{ref-2%4} removed')

        tileType = self.tileTypes.index(tuple(moveOptions))

        self.maze[coords] = Tile(tileType, coords, referenceList, len(self.maze))

        return(coords)

    def generate_next(self, direction, oldCoords):
        tile = self.maze[oldCoords]
        coords = tile.reference[direction]
        
        referenceList = [(coords[0] + n, coords[1] + m)
                        for n, m in ((0,1),(1,0),(0,-1),(-1,0))]
        inverseDirection = referenceList.index(oldCoords)
        if self.tileTypes[tile.type][direction]:
            tileOptions = [n for n in range(15)
                           if self.tileTypes[n][inverseDirection]]
        else:
            tileOptions = [n for n in range(15)]
            
        for d in range(4):
            ref = self.maze.get(referenceList[d])
            if ref:
                is_ref_clear = self.tileTypes[ref.type][(d+2)%4]
                print(is_ref_clear)
                if not is_ref_clear:
                    n=0
                    while n < len(tileOptions):
                        op = tileOptions[n]
                        if self.tileTypes[op][d]:
                            tileOptions.remove(op)
                            print(f'{op} removed')
                        else:
                            n += 1
        
        
        tileType = r.choice(tileOptions)
        print(tileOptions)
        self.maze[coords] = Tile(tileType, coords, referenceList, len(self.maze))
        return(coords)
